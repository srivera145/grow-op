...
| `editor-art.js` | The Art panel's audit, and what the dev server will write because of it. Four fixtures are repacked through the real route: two strips, one drawn the way the prompt asks for and one drawn every way it asks you not to, and two 3x3 tilesets, one see-through at its cell joins and one not. The flagged strip is refused an accept even when the request claims it has no flags; the seamy tileset is *not* refused, and the backing derived for it is checked to be measured off that sheet rather than copied from `tiles-soil`. No image is generated and no key is used. |
